...
class Encoder(nn.Module):
  ...
  def __init__(
    ...
    # self.node_fn = nn.Embedding(nnode_in_features, nnode_out_features)
...
class Decoder(nn.Module):
  ...
  def __init__(
    ...
    self.force_fn = build_mlp(
        nnode_in, [mlp_hidden_dim for _ in range(nmlp_layers)], nnode_out)
    self.energy_fn = build_mlp(
        nnode_in, [mlp_hidden_dim for _ in range(nmlp_layers)], output_size=1)
  ...
  def forward(self,
    ...
    """
    return self.energy_fn(x), self.force_fn(x)
...
class EncodeProcessDecode(nn.Module):
  ...
  def forward(self,
    ...
    energy, force = self._decoder(x)
    energy = scatter(energy, batch, dim=0, reduce='add')
    return energy, force
